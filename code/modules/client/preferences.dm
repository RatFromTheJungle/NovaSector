...
/atom/movable/screen/map_view/char_preview/proc/update_body()
	...
	// BUBBER STATION EDIT NOVA: Better character preview
	...
	if (last_canvas_size != canvas_size || last_canvas_state != canvas_state)
		...
		switch(canvas_size)
			if(0)
				body.pixel_x = 0
				canvas = image('modular_nova/master_files/icons/customization/template.dmi', icon_state = canvas_state)
			if(1)
				body.pixel_x = 16
				canvas = image('modular_nova/master_files/icons/customization/template_64x64.dmi', icon_state = canvas_state)
			else
				body.pixel_x = 32
				canvas = image('modular_nova/master_files/icons/customization/template_96x96.dmi', icon_state = canvas_state)
